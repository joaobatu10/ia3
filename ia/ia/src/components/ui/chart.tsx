  ...
  const css = Object.entries(THEMES)
    .map(([theme, prefix]) => {
      const body = colorConfig
        .map(([key, itemConfig]) => {
          const color =
            (itemConfig as any).theme?.[theme as keyof typeof THEMES] || (itemConfig as any).color;
...
function ChartTooltipContent(props: any) {
  // usando `any` para simplificar a integração com as props do Recharts
  const {
    active,
    payload,
    className,
...
function getPayloadConfigFromPayload(config: ChartConfig, payload: any, key: string) {
  ...
  // prefer exact match by computed key, otherwise fallback to key
